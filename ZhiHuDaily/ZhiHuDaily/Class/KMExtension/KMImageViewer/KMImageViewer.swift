...
class KMImageViewer: UIViewController {
    ...
    fileprivate var dissmissCallback : (() -> Void)?
    
    var didCreateUI : Bool = false
    ...
    public init(showByViewController presentingVC: UIViewController, imageURL: URL?, relatedView: UIView, dissmiss: (() -> Void)?) {
    ...
    public class func show(byViewController presentingVC: UIViewController, imageURL: URL?, relatedView: UIView, dissmiss: (() -> Void)?) {
        let vc = KMImageViewer(showByViewController: presentingVC, imageURL: imageURL, relatedView: relatedView, dissmiss: dissmiss)
        vc.show()
    }
    
    deinit {
        #if DEBUG
            if dissmissCallback != nil {
                dissmissCallback!()
            }
            print("deinit:\(self)")
        #endif
...
fileprivate extension KMImageViewer {
    func createUI() {
        ...
        doLayout()
        loadImage(withPlaceholder: #imageLiteral(resourceName: "Field_Mask_Bg"), url: imageURL)
    }
...
//MARK: - @objc
@objc fileprivate extension KMImageViewer {
    /// 响应单击
    func onSingleTap() {
        handelSingleTap()
    }
